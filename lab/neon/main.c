...
int main(int argc, char *argv[])
{
    ...
    int left_over = (rows * cols) % 4;
    for (int i = 0; i < (rows * cols)-left_over; i+=4)
    {
        float32x4_t quadword = vld1q_f32(host_a.data + i);
        sum += vaddvq_f32(quadword);
    }

    for (int i = (rows * cols)-left_over; i < (rows * cols) ;i++)
    {
        sum += host_a.data[i];
    }
