...
void NaiveMatrixMultiply(Matrix *input0, Matrix *input1, Matrix *result)
{
    ...
    for (int row = 0; row < i0_row; row++)
    {
                ...
                for (int blockCol = col; blockCol < blockColLim; blockCol+=4)
		{
			...
			float data0quad[4] = {0, 0, 0, 0};
                        data0quad[0] = (*input0).data[i0_pos];
                        data0quad[1] = (*input0).data[i0_pos + 1];
                        data0quad[2] = (*input0).data[i0_pos + 2];
                        data0quad[3] = (*input0).data[i0_pos + 3];
                        float32x4_t data0 = vld1q_f32(data0quad);

                        float data1quad[4] = {0, 0, 0, 0};
                        data1quad[0] = (*input1).data[i1_pos];
                        data1quad[1] = (*input1).data[i1_pos + i1_col];
                        data1quad[2] = (*input1).data[i1_pos + i1_col * 2];
                        data1quad[3] = (*input1).data[i1_pos + i1_col * 3];
                        float32x4_t data1 = vld1q_f32(data1quad);

                        float32x4_t data = vmulq_f32(data0, data1);
                        result->data[r_pos] += vaddvq_f32(data);
